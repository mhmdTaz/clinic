 ...
 * deploy.
 */
test('a role made at runtime reaches its member on her very next request', async ({
  page,
  browser,
}) => {
  const roleName = `Head Nurse ${Date.now()}`

  // Hana is already signed in, with no role: no portal, and the patient API refuses her.
  ...
  // Her next API request runs with the new grants…
  expect((await nurse.request.get('/api/v1/patients')).status()).toBe(200)

  // …and her next page load opens the staff portal, with Patients in the menu.
  await nurse.reload()
